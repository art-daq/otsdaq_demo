    ...
    if (FD_ISSET(sockfd, &readfds)) {
      ...
      while (handlerIndex < totalNumberOfBytes &&
            ...
            buff[handlerIndex + 0] == 0)  // read
        {
          ...
          if ((numberOfBytes = sendto(sockfd, buff, packetSz, 0, (struct sockaddr *)&their_addr,
                   ...
                   buff[handlerIndex + 0] == 1)                            // write
        {
          ...
          switch (addr)  // define address space
          {
            ...
            case 0x1003:
              ...
              // show "LEDs"
              __COUT__ << "\n\n";
              for (int l = 0; l < 8; ++l) __COUT__ << ((led_register & (1 << (7 - l))) ? '*' : '-');
              __COUT__ << "\n\n";
              break;
            ...
            case 0x0000000100000008: {
              ...
              if (sendSockfd != -1) {
                ...
                __COUT__ << std::hex << ":::"
                         << "Failed to create streaming socket to ip: " << streamToIP << " port: 0x" << streamToPort
                         << endl;
          ...
          __COUT__ << std::hex << ":::"
                   << "ERROR: The formatting of the string received is wrong! Number of bytes: " << numberOfBytes
                   << " Read/Write " << buff[handlerIndex + 0] << std::endl;
      ...
      if ((int)dataEnabled) {
        // generate data
        //__COUT__ << "[" << __LINE__ << "]Count? " << count << " rate: " << data_gen_rate << " counter: " <<
        //data_gen_cnt << endl;
            ...
            data_gen_cnt != 0)             // still packets to send
        {
          // if(count%0x100000 == 0)
          __COUT__ << std::hex << ":::"
                   << "Count: " << count << " rate: " << data_gen_rate << " packet-counter: " << data_gen_cnt
                   << std::endl;
          __COUT__ << std::hex << ":::"
          ...
          if (data_gen_cnt != (uint64_t)-1) --data_gen_cnt;
        }
