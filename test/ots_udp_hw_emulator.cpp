...
int main(int argc, char** argv)
{
	...
	while(1)
	{
		...
		{
			//packet received
			__COUT__ << std::hex << ":::" << "Packet Received!" << std::endl;

			addr_len = sizeof their_addr;
			if ((totalNumberOfBytes = recvfrom(sockfd, buff, MAXBUFLEN-1 , 0,	(struct sockaddr *)&their_addr, &addr_len))
					== -1)
			{
				perror("recvfrom");
				exit(1);
			...
			__COUT__ << ":::" << "hw: got packet from " << inet_ntop(their_addr.ss_family, get_in_addr((struct sockaddr *)&their_addr),	s, sizeof s) << std::endl;
			__COUT__ << ":::" << "hw: packet total is " << totalNumberOfBytes << " bytes long" << std::endl;
			...
			{
				__COUT__ << ":::" << "hw: packet byte index " << handlerIndex << std::endl;
				__COUT__ << ":::" << "hw: packet is " << numberOfBytes << " bytes long" << std::endl;
						...
						buff[handlerIndex + 0] == 1) //write
				{
					...
					case 0x0000000100000006:
					{
						...
						__COUT__ << std::hex << ":::" << "Stream destination IP: " << streamToIP << std::endl;
						__COUT__ << streamToIP << std::endl;
					}
						break;
					case 0x0000000100000007:
						__COUT__ << std::hex << ":::" << "Destination MAC address ignored!" << std::endl;
						break;
					case 0x0000000100000008:
					{
						//unsigned int myport;
						memcpy((void *)&streamToPort,(void *)&buff[handlerIndex + RX_DATA_OFFSET],4);
						__COUT__ << std::hex << ":::" << "Stream destination port: 0x" << streamToPort << dec << " " << streamToPort << endl;
						...
						if(sendSockfd != -1)
						{
							__COUT__ << "********************************************************" << endl;
							__COUT__ << "********************************************************" << endl;
							__COUT__ << std::hex << ":::" << "Streaming to ip: " << streamToIP << " port: 0x" << streamToPort << endl;
							__COUT__ << "********************************************************" << endl;
							__COUT__ << "********************************************************" << endl;
						}
						else
							__COUT__ << std::hex << ":::" << "Failed to create streaming socket to ip: " << streamToIP << " port: 0x" << streamToPort << endl;
					...
					case 0x0000000100000009:
						...
						break;
					default:
						__COUT__ << std::hex << ":::" << "Unknown write address received." << endl;
					}
				}
				else
					__COUT__ << std::hex << ":::" << "ERROR: The formatting of the string received is wrong! Number of bytes: " << numberOfBytes << " Read/Write " << buff[handlerIndex + 0] << std::endl;
		...
		else
		{
			...
			if((int)dataEnabled)
			{
				...
				{
					//if(count%0x100000 == 0)
					__COUT__ << std::hex << ":::" << "Count: " << count << " rate: " << data_gen_rate << " packet-counter: " << data_gen_cnt << std::endl;
					__COUT__ << std::hex << ":::" << "Send Burst at count:" << count << std::endl;
					//send a packet
					...
					__PRINTF__("hw: sent %d bytes back. sequence=%d\n",
							numberOfBytes, (unsigned char)buff[1]);
			...
			{
				wasDataEnable = false;
				__COUT__ << std::hex << ":::" << "Send Last in Burst at count:" << count << endl;
				//send a packet
				...
				if(sendSockfd != -1)
				{
					...
					__COUT__ << std::hex << ":::" << "Send socket not defined." << endl;
			}
