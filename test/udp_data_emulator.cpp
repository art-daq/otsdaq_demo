...
int makeSocket(const char* ip, const char* port, struct addrinfo*& addressInfo) {
  ...
  if (ip == NULL) hints.ai_flags = AI_PASSIVE;  // use my IP
...
int send(int toSocket, struct sockaddr_in& toAddress, const std::string& buffer) {
  std::cout << "Socket Descriptor #: " << toSocket << " ip: " << std::hex << toAddress.sin_addr.s_addr << std::dec
            << " port: " << ntohs(toAddress.sin_port) << std::endl;
  if (sendto(toSocket, buffer.c_str(), buffer.size(), 0, (struct sockaddr*)&(toAddress), sizeof(sockaddr_in)) <
      (int)(buffer.size())) {
...
int receive(int fromSocket, struct sockaddr_in& fromAddress, std::string& buffer) {
  ...
  if (FD_ISSET(fromSocket, &fileDescriptor)) {
    ...
    if ((nOfBytes = recvfrom(fromSocket, &buffer[0], MAXBUFLEN, 0, (struct sockaddr*)&fromAddress, &addressLength)) ==
        -1)
      return -1;
