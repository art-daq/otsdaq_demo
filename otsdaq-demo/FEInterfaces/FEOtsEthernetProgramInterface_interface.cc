...
//========================================================================================================================
FEOtsEthernetProgramInterface::FEOtsEthernetProgramInterface(const std::string &interfaceUID,
  ...
  if ((pDIR = opendir(dirpath.c_str()))) {
    ...
    while ((entry = readdir(pDIR))) {
      ...
      if (entry->d_name[0] != '.' && (entry->d_type == 0 ||  // 0 == UNKNOWN (which can happen - seen in SL7 VM)
        ...
        __COUT__ << int(entry->d_type) << " " << entry->d_name << " " << std::string(entry->d_name).find(".bin") << " "
...
void FEOtsEthernetProgramInterface::loadProgramFile(frontEndMacroConstArgs_t argsIn, frontEndMacroArgs_t argsOut) {
  ...
  // setup address and flash write mode
  {
    ...
    sendBuffer.resize(0);
    OtsUDPFirmwareCore::writeAdvanced(sendBuffer, UDP_CORE_BLOCK_ADDRESS /*block*/ | FLASH_COMMAND_BASE /*addr*/,
                                      dataVec /*data*/);
    OtsUDPHardware::write(sendBuffer);
  }
  ...
  while (1) {
    ...
    if (result != page_size)  // if block of data is not full page size, handle it.
    {
      ...
      {
        for (unsigned int i = result; i < page_size; i++) page_data[i] = 0;
      }
    }
    ...
    if (page > 1)  // if 3rd page and beyond, handle differently
    {
      ...
      // check status
      sendBuffer.resize(0);
      OtsUDPFirmwareCore::read(sendBuffer, UDP_CORE_BLOCK_ADDRESS /*block*/ | FLASH_WRITE_STATUS /*addr*/);
      ...
      //			returnaddr = 0 | (int)next_page_recv_buff[21]<<16 | (int)next_page_recv_buff[20]<<8 |
      //(int)next_page_recv_buff[19]; 			thisbytesoff = ftell(bitstream) - returnaddr;
    ...
    // ready, so send data
    sendBuffer.resize(0);
    OtsUDPFirmwareCore::writeAdvanced(sendBuffer, UDP_CORE_BLOCK_ADDRESS /*block*/ | FLASH_WRITE_DATA /*addr*/,
