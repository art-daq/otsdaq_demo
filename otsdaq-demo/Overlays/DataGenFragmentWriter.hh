...
ots::DataGenFragmentWriter::DataGenFragmentWriter(artdaq::Fragment &f) : DataGenFragment(f), artdaq_Fragment_(f) {
  if (!f.hasMetadata() || f.dataSizeBytes() > 0) {
    throw cet::exception(
        "Error in DataGenFragmentWriter: Raw artdaq::Fragment object does not appear to consist of (and only of) its "
        "own header + the DataGenFragment::Metadata object");
  }
...
inline void ots::DataGenFragmentWriter::resize(size_t nBytes) { artdaq_Fragment_.resizeBytes(nBytes); }
