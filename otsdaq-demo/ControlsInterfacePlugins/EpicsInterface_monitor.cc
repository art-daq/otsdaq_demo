...
// g++ -std=c++11  EpicsCAMonitor.cpp EpicsCAMessage.cpp EpicsWebClient.cpp SocketUDP.cpp SocketTCP.cpp
// -L$EPICS_BASE/lib/linux-x86_64/ -Wl,-rpath,$EPICS_BASE/lib/linux-x86_64 -lca -lCom -I$EPICS_BASE//include
// -I$EPICS_BASE//include/os/Linux -I$EPICS_BASE/include/compiler/gcc -o EpicsWebClient
...
EpicsInterface::EpicsInterface() {
  ...
  SEVCHK(ca_context_create(ca_enable_preemptive_callback),
         "EpicsInterface::EpicsInterface() : ca_enable_preemptive_callback_init()");
}
...
std::string EpicsInterface::getList(std::string format) {
  ...
  if (format == "JSON") {
    // pvList = "{\"PVList\" : [";
    pvList = "[";
    for (auto it = mapOfPVInfo_.begin(); it != mapOfPVInfo_.end(); it++) pvList += "\"" + it->first + "\", ";
  ...
  if (eha.status == ECA_NORMAL) {
    ...
    switch (eha.type) {
      case DBR_CTRL_CHAR:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVControlValueToRecord(
                ca_name(eha.chid), ((struct dbr_ctrl_char *)eha.dbr));  // write the PV's control values to records
        break;
      case DBF_DOUBLE:
        if (DEBUG) {
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVValueToRecord(ca_name(eha.chid),
      ...
      case DBR_STS_STRING:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->sstrval.status],
...
printf("\n");
      ...
      case DBR_STS_SHORT:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->sshrtval.status],
...
printf("\n");
      ...
      case DBR_STS_FLOAT:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->sfltval.status],
...
printf("\n");
      ...
      case DBR_STS_ENUM:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->senmval.status],
...
printf("\n");
      ...
      case DBR_STS_CHAR:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->schrval.status],
...
printf("\n");
      ...
      case DBR_STS_LONG:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->slngval.status],
...
printf("\n");
      ...
      case DBR_STS_DOUBLE:
        ...
        ((EpicsInterface *)eha.usr)
            ->writePVAlertToQueue(ca_name(eha.chid), epicsAlarmConditionStrings[pBuf->sdblval.status],
...
printf("\n");
      ...
      default:
        if (ca_name(eha.chid)) {
          ...
          ((EpicsInterface *)eha.usr)
              ->writePVValueToRecord(ca_name(eha.chid), (char *)eha.dbr);  // write the PV's value to records
        }
...
ca_element_count(cha.chid), cha.chid, eventCallback, this); SEVCHK(status_, "ca_array_get_callback");*/
...
bool EpicsInterface::checkIfPVExists(std::string pvName) {
  ...
  if (mapOfPVInfo_.find(pvName) != mapOfPVInfo_.end()) return true;
...
void EpicsInterface::getControlValues(std::string pvName) {
  ...
  SEVCHK(ca_array_get_callback(DBR_CTRL_CHAR, 0, mapOfPVInfo_.find(pvName)->second->channelID, eventCallback, this),
...
void EpicsInterface::createChannel(std::string pvName) {
  ...
  std::cout << "Trying to create channel to " << pvName << ":" << mapOfPVInfo_.find(pvName)->second->channelID
            << std::endl;

  if (mapOfPVInfo_.find(pvName)->second !=
      NULL)  // Check to see if the pvName maps to a null pointer so we don't have any errors
    if (mapOfPVInfo_.find(pvName)->second->channelID !=
        NULL)  // channel might exist, subscription doesn't so create a subscription
    {
      ...
      if (DEBUG) {
        std::cout << "Channel to " << pvName << " exists, but is not connected! Destroying current channel."
                  << std::endl;
      }
  ...
  SEVCHK(
      ca_create_channel(pvName.c_str(), staticChannelCallbackHandler, mapOfPVInfo_.find(pvName)->second->parameterPtr,
                        0, &(mapOfPVInfo_.find(pvName)->second->channelID)),
...
void EpicsInterface::subscribeToChannel(std::string pvName, chtype subscriptionType) {
  ...
  if (DEBUG) {
    std::cout << "Trying to subscribe to " << pvName << ":" << mapOfPVInfo_.find(pvName)->second->channelID
              << std::endl;
  }
  ...
  SEVCHK(ca_create_subscription(dbf_type_to_DBR(mapOfPVInfo_.find(pvName)->second->channelType), 1,
                                mapOfPVInfo_.find(pvName)->second->channelID, DBE_VALUE | DBE_ALARM | DBE_PROPERTY,
                                eventCallback, this, &(mapOfPVInfo_.find(pvName)->second->eventID)),
...
void EpicsInterface::readValueFromPV(std::string pvName) {
  ...
  // &(mapOfPVInfo_.find(pvName)->second->pvValue), eventCallback, &(mapOfPVInfo_.find(pvName)->second->callbackPtr)),
  // "ca_get");
...
void EpicsInterface::debugConsole(std::string pvName) {
  ...
  for (unsigned int it = 0; it < mapOfPVInfo_.find(pvName)->second->dataCache.size() - 1; it++) {
    ...
    std::cout << "Iteration: " << it << " | " << mapOfPVInfo_.find(pvName)->second->mostRecentBufferIndex << " | "
  ...
  if (mapOfPVInfo_.find(pvName) != mapOfPVInfo_.end()) {
    if (mapOfPVInfo_.find(pvName)->second !=
          ...
          NULL)  // channel might exist, subscription doesn't so create a subscription
      {
        ...
        std::string units, upperDisplayLimit, lowerDisplayLimit, upperAlarmLimit, upperWarningLimit, lowerWarningLimit,
    ...
    // std::array<std::string, 9> s = {units, upperDisplayLimit, lowerDisplayLimit, upperAlarmLimit, upperWarningLimit,
    // lowerWarningLimit, lowerAlarmLimit, upperControlLimit, lowerControlLimit};
