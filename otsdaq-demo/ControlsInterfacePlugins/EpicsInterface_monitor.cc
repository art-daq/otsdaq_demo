...
// g++ -std=c++11  EpicsCAMonitor.cpp EpicsCAMessage.cpp EpicsWebClient.cpp SocketUDP.cpp
// SocketTCP.cpp -L$EPICS_BASE/lib/linux-x86_64/ -Wl,-rpath,$EPICS_BASE/lib/linux-x86_64
// -lca -lCom -I$EPICS_BASE//include -I$EPICS_BASE//include/os/Linux
// -I$EPICS_BASE/include/compiler/gcc -o EpicsWebClient
...
void EpicsInterface::loadListOfPVs()
{
	...
	// get parameters (e.g. HIHI("upper alarm") HI("upper warning") LOLO("lower alarm"))
	// for each pv
	for(auto pv : mapOfPVInfo_)
	{
...
void EpicsInterface::createChannel(std::string pvName)
{
	...
	if(mapOfPVInfo_.find(pvName)->second != NULL)  // Check to see if the pvName maps to a
	                                               // null pointer so we don't have any
	                                               // errors
...
void EpicsInterface::cancelSubscriptionToChannel(std::string pvName)
{
	if(mapOfPVInfo_.find(pvName)->second != NULL)
		if(mapOfPVInfo_.find(pvName)->second->eventID != NULL)
		{
			...
			SEVCHK(
			    status_,
			    "EpicsInterface::cancelSubscriptionToChannel() : ca_clear_subscription");
			if(status_ == ECA_NORMAL)
			{
...
std::array<std::string, 9> EpicsInterface::getSettings(std::string pvName)
{
	...
	if(mapOfPVInfo_.find(pvName) != mapOfPVInfo_.end())
	{
		if(mapOfPVInfo_.find(pvName)->second != NULL)  // Check to see if the pvName maps
		                                               // to a null pointer so we don't
		                                               // have any errors
			if(mapOfPVInfo_.find(pvName)->second->channelID !=
		...
		// std::array<std::string, 9> s = {units, upperDisplayLimit, lowerDisplayLimit,
		// upperAlarmLimit, upperWarningLimit, lowerWarningLimit, lowerAlarmLimit,
		// upperControlLimit, lowerControlLimit};
