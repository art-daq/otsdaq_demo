    ...
    for (size_t word = 0; word < (*packet)->size(); word += 4) {
      ...
      if ((byte0 & 0x8) == 0) {
        ...
        if ((byte0 & 0x10) == 0x10) {  // Bunch Counter Low
          ...
          bunch_counter =
              (bunch_counter & 0xFFFFFF) + ((uint64_t)byte1 << 24) + ((uint64_t)byte2 << 32) + ((uint64_t)byte3 << 40);
        } else if ((byte0 & 0xb0) == 0xb0) {  // Trigger Counter High
          triggerCounter =
              (triggerCounter & 0xFFFF) + ((uint64_t)byte1 << 16) + ((uint64_t)byte2 << 24) + ((uint64_t)byte3 << 32);
