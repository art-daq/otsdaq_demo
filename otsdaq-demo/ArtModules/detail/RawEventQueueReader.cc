...
ots::detail::RawEventQueueReader::RawEventQueueReader(fhicl::ParameterSet const& ps, art::ProductRegistryHelper& help,
  ...
  for (auto& name : names) {
    TLOG(TLVL_INFO, "RawEventQueueReader")
        << "Adding fragment type " << name << " to fragment_type_map, and registering with the ProductRegistryHelper";
