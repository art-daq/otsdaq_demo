...
double ots::WFViewer::calcmean(const float *data) {
  ...
  mean /= 10;
  std::cout << "WFViewer mean is " << mean << " data[0] is " << data[0] << " data[1] is " << data[1] << " 2=" << data[2]
            << " 3=" << data[3] << std::endl
            << std::endl;
  std::cout << "WFViewer mean is " << mean << " data[15997] is " << data[15997] << " data[15998] is " << data[15998]
            << " 2=" << data[15999] << " 3=" << data[15999] << std::endl
            << std::endl;
  //"Hex[0]:" << std::hex << lp[0] << " [1]:" << std::hex << lp[1] << " [2]:" << std::hex << lp[2] << " [3]:" <<
  //std::hex << lp[3] << " [4]:" << std::hex << lp[4] << std::endl << std::endl << std::endl;
  return mean;
}
...
void ots::WFViewer::analyze(art::Event const &e) {
  ...
  for (auto label : fragment_type_labels_) {
    ...
    // std::cout << __COUT_HDR_FL__ << "WFViewer: There are " << (*fragments_with_label).size() << " fragments in this
    // event" << std::endl;
  ...
  for (const auto &frag : fragments) {
    ...
    if (expected_sequence_id != frag.sequenceID()) {
      TLOG(TLVL_WARNING, "WFViewer") << "Warning in WFViewer: expected fragment with sequence ID "
                                     << expected_sequence_id << ", received one with sequence ID " << frag.sequenceID();
    }

    FragmentType fragtype = static_cast<FragmentType>(frag.type());
    // std::cout << __COUT_HDR_FL__ << "WFViewer: Fragment type is " << fragtype << " (DataGen=" <<
    // FragmentType::DataGen << ")" << std::endl;
    ...
    if (!histograms_[ind]) {
      histograms_[ind] = std::unique_ptr<TH1D>(new TH1D(Form("Fragment_%d_hist", fragment_id), "Title Dennis", 300,
    ...
    switch (fragtype) {
      case FragmentType::DataGen:
        // for (auto val = drPtr->dataBegin(); val <= drPtr->dataEnd(); ++val )
        {
          ...
          std::cout << __COUT_HDR_FL__ << "DJN WFViewer: Putting datapoint " << the_mean << " into histogram"
                    << std::endl;
