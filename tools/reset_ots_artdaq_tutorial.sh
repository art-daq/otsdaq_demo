...
echo -e `date +"%h%y %T"` "reset_ots_tutorial.sh [${LINENO}]  \t Please do not source this script, run it as ./reset_ots_artdaq_tutorial.sh"
...
if [[ $? -eq 1 ]];then #no
	...
	#echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t kdialog is not functional, attempt to bypass with alias echo and KDIALOG_ALWAYS_YES"
	echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t kdialog is not functional, bypassing user prompts"	
	echo
	...
	echo
	echo
	echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t Tutorial reset script complete."
...
if [[ $KDIALOG_ALWAYS_YES == 0 && $? -eq 1 ]]; then #no
	echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t User decided to not continue with starting the tutorial. Exiting script."
...
if [[ $KDIALOG_ALWAYS_YES == 1 || $? -eq 0 ]];then #yes
	...
	echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t User decided to reset to 'artdaq Demo' tutorial data."
	...
	if [ "x$USER_DATA" == "x" ]; then
		...
		echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t Error! You must already have ots setup (i.e. $USER_DATA must point to the right place)... For example, export USER_DATA=$MRB_SOURCE/otsdaq_demo/NoGitData. Exiting script."
	...
	if [ "x$ARTDAQ_DATABASE_URI" == "x" ]; then
		...
		echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t Error! You must already have ots setup (i.e. $ARTDAQ_DATABASE_URI must point to the right place)... For example, export USER_DATA=filesystemdb://$MRB_SOURCE/otsdaq_demo/NoGitDatabases/filesystemdb/test_db. Exiting script."
    ...
    echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t Now your user data path is USER_DATA = ${USER_DATA}"
    echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t Now your database path is ARTDAQ_DATABASE_URI = ${ARTDAQ_DATABASE_URI}"
...
if [[ $KDIALOG_ALWAYS_YES == 1 || $? -eq 1 ]];then #no
	echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t User decided to not start the tutorial. Exiting script."
...
echo -e `date +"%h%y %T"` "reset_ots_artdaq_tutorial.sh [${LINENO}]  \t User decided to start up the tutorial."
